// Errors.h — Serial command error codes
//
// Defines every error code that the firmware can report via the GERR command.
// Only the most-recent error is retained in ErrorCode; it is never cleared so
// a host can retrieve it at any time.  Use SetErrorCode(x) to record an error
// and SendNAK to notify the host; the BADARG / ERR macros combine both steps.
//
// Originally created by Gordon Anderson, 9/7/14.
